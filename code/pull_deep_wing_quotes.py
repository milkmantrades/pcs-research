#!/usr/bin/env python3
"""Pull put quotes at deep ATR fibs (1.236, 1.618, 1.786) for the wing variants.

Weeks < 2020-05-01 use spx_weekly_levels_full.csv (backfill convention);
weeks >= 2020-05-01 use canonical spx_weekly_levels.csv. Same DB; qdone dedups.
"""
import csv
import io
import sqlite3
...
def main():
    ...
    for wend, row in wk.iterrows():
        ...
        if settle not in exps:
            continue
        for fib in DEEP_FIBS:
            k = round5(row["prev_close"] - fib * row["atr_prev"])
            if (entry, settle, float(k), "P") not in done:
                plan.append((entry, settle, k, "P"))
                done.add((entry, settle, float(k), "P"))
